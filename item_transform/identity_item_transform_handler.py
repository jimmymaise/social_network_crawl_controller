...
class IdentityItemTransformHandler(BaseItemTransformHandler):
    ...
    def process_item(self, load_items, crawl_items):
        _user_data = self.get_users_data(load_items, crawl_items)
        _post_data = self.get_posts_data(load_items, crawl_items)
        ...
        user_objs = self.build_user_object(user_data)
        post_objs = self.build_user_object(user_data)

        return user_objs, post_objs

    @staticmethod
    def get_users_data(load_items, crawl_items):
        user_objs = {}

        return user_objs

    @staticmethod
    def get_posts_data(load_items, crawl_items):
        post_objs = {}
    ...
    @staticmethod
    def build_post_object(post_data):
        _post_objs = {}
